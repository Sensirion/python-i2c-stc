...
class Stc3xI2cCmdSetBinaryGas(Stc3xI2cCmdBase):
    ...
    def __init__(self, binary_gas):
        ...
        :param int binary_gas:
            ...
            available binary gases. STC31: - 0x0000: CO₂ in N₂ for range in 0
            to 100 vol% - 0x0001: CO₂ in air for range in 0 to 100 vol% -
            0x0002: CO₂ in N₂ for range in 0 to 25 vol% - 0x0003: CO₂ in air
            for range in 0 to 25 vol%
        """
...
class Stc3xI2cCmdSetPressure(Stc3xI2cCmdBase):
    ...
    def __init__(self, absolue_pressure):
        ...
        super(Stc3xI2cCmdSetPressure, self).__init__(
            command=0x362F,
            tx_data=b"".join([pack(">H", absolue_pressure)]),
            rx_length=None,
            read_delay=0.0,
            timeout=0,
            post_processing_time=0.001,
...
class Stc3xI2cCmdMeasureGasConcentration(Stc3xI2cCmdBase):
    ...
    def interpret_response(self, data):
        ...
        :return:
            - gas_ticks (int) -
              Gas concentration. Convert to val % by 100 \* (value - 2^14) /
              2^15
            - temperature_ticks (int) -
...
class Stc3xI2cCmdForcedRecalibration(Stc3xI2cCmdBase):
    ...
    def __init__(self, reference_concentration):
        ...
        :param int reference_concentration:
            Reference concentration
        """
        super(Stc3xI2cCmdForcedRecalibration, self).__init__(
            command=0x3661,
            tx_data=b"".join([pack(">H", reference_concentration)]),
            rx_length=None,
            read_delay=0.0,
            timeout=0,
...
class Stc3xI2cCmdGetSensorState(Stc3xI2cCmdBase):
    ...
    def interpret_response(self, data):
        ...
        # convert raw received data into proper data types
        state = [int(ii) for ii in unpack(">{}B".format(len(checked_data[0:30]) // 1), checked_data[0:30])]  # list(uint8)
        return state
...
class Stc3xI2cCmdSelfTest(Stc3xI2cCmdBase):
    ...
    State ------------------------------- 0-1: \| Memory error 2 : \| VDD out
    of range 3-8: \| Measurement value error ------------------------------- In
    case of a successful self-test the sensor returns 0x0000 with correct CRC.
    """
